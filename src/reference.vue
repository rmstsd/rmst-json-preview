  ...
  <ul ref="scroller" class="height-dynamic" @scroll="handleScroll">
    ...
    <place-holder
      ...
      :style="`transform: translate(0, ${
        cachedScrollY[firstAttachedItem] - ESTIMATED_HEIGHT * (index + 1)
      }px)`"
    />
    <Item
    ...
    <place-holder
      ...
      :key="index + 1"
      :style="`transform: translate(0, ${
        cachedScrollY[lastAttachedItem - 1] +
    ...
    methods: {
      ...
      async updateAnchorItem(delta) {
        ...
        if (this.cachedScrollY[this.firstAttachedItem] <= -1) {
          ...
          this.revising = true
          const actualScrollY = this.cachedHeight
            .slice(0, Math.max(0, this.anchorItem.index))
      ...
      async calItemScrollY() {
        ...
        this.$set(
          ...
          this.$refs.scroller.scrollTop - this.anchorItem.offset
        )
      ...
      updatePlaceholder(isPositive) {
        if (isPositive) {
          ...
          this.bottomPlaceholders = Math.min(
            PLACEHOLDER_COUNT,
            Math.abs(this.listData.length - this.lastAttachedItem)
          )
        } else {
      ...
      handleLoadMore() {
        ...
        ;(scrollEnd >= this.scrollRunwayEnd || this.anchorItem.index === this.listData.length - 1) &&
          this.fetchData()
      }
    },
